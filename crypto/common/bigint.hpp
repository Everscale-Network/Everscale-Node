...
class BigIntG {
 ...
 public:
  ...
  bool mod_div_bool(const BigIntG<len2, Tr>& y, BigIntG<len3, Tr>& quot, int round_mode = -1) {
    BigIntG<len2, Tr> y_norm = y;
    y_norm.normalize();
    this->normalize();
    if (y_norm.as_any_int().size() == 1) {
      auto q = quot.as_any_int();
      return as_any_int().mod_div_any_small_divisor(y_norm.as_any_int(), q, round_mode);
    } else {
      BigIntG<len2, Tr> copy_this = *this;
      auto q = quot.as_any_int();
      BigIntG<len2, Tr> y_abs = y_norm;
      y_abs.mul_short(y_abs.sgn_un());
      y_abs.normalize();
      const auto result = as_any_int().mod_div_any_big_divisor(y_norm.as_any_int(), y_abs.as_any_int(), q, round_mode);
      if (!result) {
        return false;
      }
...
bool AnyIntView<Tr>::mod_div_any_big_divisor(const AnyIntView<Tr>& divisor, const AnyIntView<Tr>& divisor_abs, AnyIntView<Tr>& quot, int round_mode) {
  ...
  const auto to_bignum = [](const AnyIntView<Tr>& src_abs, arith::Bignum& dst_abs) {
    size_t bytes = (src_abs.bit_size_any(false) + 7) / 8;
    std::array<unsigned char, BigInt256::DoubleInt::max_bits / 8 + 1> binary;
    if (bytes > binary.size()) {
      return false;
    }
    src_abs.export_bytes_any(binary.data(), bytes, false);
    dst_abs.import_msb(binary.data(), bytes);
    return true;
  };
  
  const auto from_bignum = [](arith::Bignum& src, AnyIntView<Tr>& dst) {
    bool sgnd = src.sign() < 0;
    src = abs(src);
    ...
    const bool res = dst.import_bytes_any(binary.data(), bytes, src.sign() < 0);
    if (sgnd) {
      dst.mul_add_short_any(-1, 0);
      dst.normalize_bool_any();
    }
    return res;
  };
  ...
  try {
    arith::Bignum dividend_abs_bn;
    arith::Bignum divisor_abs_bn;
    if (!to_bignum(*this, dividend_abs_bn)) {
      return false;
    }
    if (!to_bignum(divisor_abs, divisor_abs_bn)) {
      return false;
    }
    arith::Bignum remainder_bn = dividend_abs_bn.divmod(divisor_abs_bn);
    arith::Bignum& quot_bn = dividend_abs_bn;
    
    if (round_mode == 0) {
    // nearest
      remainder_bn *= sign_dividend;
      if (sign_divisor < 0) {
        quot_bn *= -1 * sign_dividend;
      } else {
        quot_bn *= sign_dividend;
      }
      if (remainder_bn != 0) {
        arith::Bignum r_x2 = remainder_bn * 2;
        const int cmp_result = cmp(abs(r_x2), divisor_abs_bn);
        const bool is_not_negative = sign_dividend == sign_divisor;
        if (cmp_result > 0 || (cmp_result == 0 && is_not_negative)) {
          if (sign_divisor == remainder_bn.sign()) {
            remainder_bn = remainder_bn - divisor_abs_bn * sign_divisor;
          } else {
            remainder_bn = remainder_bn + divisor_abs_bn * sign_divisor;
          }
          if (is_not_negative) {
      ...
      if (sign_dividend >= 0 && sign_divisor > 0) {
        ...
        if (remainder_bn == 0) {
          ...
          quot_bn = (0 - quot_bn) - 1;
          remainder_bn += divisor_abs_bn * sign_divisor;
        }
      } else if (sign_dividend < 0 && sign_divisor > 0) {
        if (remainder_bn == 0) {
          quot_bn.negate();
          ...
          quot_bn = (0 - quot_bn) - 1;
          remainder_bn = (0 - remainder_bn) + divisor_abs_bn * sign_divisor;
        }
      } else if (sign_dividend < 0 && sign_divisor < 0) {
        remainder_bn.negate();
      }
      ...
      if (remainder_bn != 0 && remainder_bn.sign() == sign_divisor) {
        remainder_bn = remainder_bn - divisor_abs_bn * sign_divisor;
