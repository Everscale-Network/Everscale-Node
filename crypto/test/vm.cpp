...
void test_muldiv(const std::string &mul1, const std::string &mul2, const std::string &dividend) {
  const std::string code =
    ...
    dividend + " PUSHINT\n" + 
    "MULDIVR\n";
  test_run_vm(fift::compile_asm(code, "", true).move_as_ok(), 0);
}

TEST(VM, super_div_1) {
  test_muldiv("303424019600764", "1000", "67374462762615477834925");
}
TEST(VM, super_div_2) {
  test_muldiv("1", "1", "2251799813685248");
}
TEST(VM, super_div_3) {
  test_muldiv("2048", "14474011154664524427946373126085988481658748083205070504932198000989141204992", "25711008708143844408671393477458601640355247900524685364822016");
}
TEST(VM, super_div_4) {
  test_muldiv(
...
td::BigInt256::DoubleInt make_int_from_string(const std::string& s) {
  td::BigInt256::DoubleInt res;
  if (s == "NaN") {
    res.invalidate();
  } else {
    res.parse_dec(s);
...
void test_bigint_div(const std::string& dividend, const std::string& divisor, int round_mode, const std::string& ans_quot, const std::string& ans_remainder) {
  td::BigInt256::DoubleInt dividend_bn = make_int_from_string(dividend);
  td::BigInt256::DoubleInt divisor_bn = make_int_from_string(divisor);
  td::BigInt256::DoubleInt quot_bn;
  
  dividend_bn.mod_div(divisor_bn, quot_bn, round_mode);
  
  td::BigInt256::DoubleInt ans_quot_bn = make_int_from_string(ans_quot);
  td::BigInt256::DoubleInt ans_remainder_bn = make_int_from_string(ans_remainder);
...
TEST(VM, bigint_div) {
  ...
  test_bigint_div("1", "2251799813685248", -1, "0", "1");
  test_bigint_div("1", "2251799813685248", 1, "1", "-2251799813685247");
  test_bigint_div("1", "2251799813685248", 0, "0", "1");
  
  test_bigint_div("29642774844752946028434172162224104410437116074403984394101141506025761187823616", "25711008708143844408671393477458601640355247900524685364822016", 0, "1152921504606846976", "0");
}

void test_bigint_denormalized(const std::string& dividend1, const std::string& dividend2, const std::string& divisor1, const std::string& divisor2) {
  for (int mode = -1; mode <= 1; mode++) {
    td::BigInt256::DoubleInt b1 = make_int_from_string(dividend1);
    td::BigInt256::DoubleInt b2 = make_int_from_string(dividend2);
    td::BigInt256::DoubleInt dividend{0};
    dividend.add_mul(b1, b2);
    td::BigInt256::DoubleInt b3 = make_int_from_string(divisor1);
    td::BigInt256::DoubleInt b4 = make_int_from_string(divisor2);
    td::BigInt256::DoubleInt divisor{0};
    divisor.add_mul(b3, b4);
    td::BigInt256::DoubleInt quot;
    
    dividend.mod_div(divisor, quot, mode);

    divisor.normalize();
    if (divisor == td::BigInt256::DoubleInt{0}) {
      assert(!dividend.is_valid());
      assert(!quot.is_valid());
    } else {
      assert(dividend.is_valid());
      assert(quot.is_valid());
    }
...
TEST(VM, bigint_div2) { 
  test_bigint_denormalized("1", "1", "2", "1125899906842624");
  
  std::seed_seq seed2{100}; 
  std::mt19937 e2(seed2);
  std::uniform_int_distribution<int> uniform_dist_number(0, 9);
  std::uniform_int_distribution<int> uniform_dist_sign(0, 8);
  std::uniform_int_distribution<int> uniform_dist_len(1, 38);
  
  for (size_t i = 0; i < 1000000; i++) {
    const auto make_str = [&]() {
      std::string r;
      if (uniform_dist_sign(e2) == 0) {
        r += "-";
      }
      int len_str = uniform_dist_len(e2);
      for (size_t j = 0; j < len_str; j++) {
        char c = uniform_dist_number(e2) + '0';
        r += c;
      }
      return r;
    };

    test_bigint_denormalized(make_str(), make_str(), make_str(), make_str());
  }
}
